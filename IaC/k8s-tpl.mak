...
deploy: appns gw landlord property tenant db monitoring
	$(KC) -n $(APP_NS) get gw,vs,deploy,svc,pods

# --- rollout: Rollout new deployments of all microservices
rollout: rollout-landlord rollout-property rollout-tenant rollout-db

# --- rollout-landlord: Rollout a new deployment of S1
rollout-landlord: $(LOG_DIR)/landlord-$(LANDLORD_VER).repo.log  cluster/service-landlord-$(LANDLORD_VER).yaml
	$(KC) -n $(APP_NS) apply -f cluster/service-landlord-$(LANDLORD_VER).yaml | tee $(LOG_DIR)/rollout-service-landlord.log
	$(KC) rollout -n $(APP_NS) restart deployment/service-landlord-$(LANDLORD_VER) | tee -a $(LOG_DIR)/rollout-service-landlord.log

# --- rollout-property: Rollout a new deployment of property
rollout-property: property
	$(KC) rollout -n $(APP_NS) restart deployment/service-property

# --- rollout-tenant: Rollout a new deployment of S3
rollout-tenant: tenant
	$(KC) rollout -n $(APP_NS) restart deployment/service-tenant

# --- rollout-db: Rollout a new deployment of DB
rollout-db: db
	$(KC) rollout -n $(APP_NS) restart deployment/teamadb
...
health-off:
	$(KC) -n $(APP_NS) apply -f cluster/service-landlord-nohealth.yaml
	$(KC) -n $(APP_NS) apply -f cluster/service-property-nohealth.yaml
	$(KC) -n $(APP_NS) apply -f cluster/service-tenant-nohealth.yaml
...
clean:
	/bin/rm -f $(LOG_DIR)/{landlord,property,tenant,db,gw,monvs}*.log $(LOG_DIR)/rollout*.log
...
log-landlord:
	$(KC) -n $(APP_NS) logs deployment/service-landlord --container service-landlord

log-property:
	$(KC) -n $(APP_NS) logs deployment/service-property --container service-property

log-tenant:
	$(KC) -n $(APP_NS) logs deployment/service-tenant --container service-tenant

log-db:
	$(KC) -n $(APP_NS) logs deployment/teamadb --container teamadb


# --- shell-X: hint for shell into a particular service
shell-landlord:
	@echo Use the following command line to drop into the landlord service:
	@echo   $(KC) -n $(APP_NS) exec -it deployment/service-landlord --container service-landlord -- bash

shell-property:
	@echo Use the following command line to drop into the property service:
	@echo   $(KC) -n $(APP_NS) exec -it deployment/service-property --container service-property -- bash

shell-tenant:
	@echo Use the following command line to drop into the tenant service:
	@echo   $(KC) -n $(APP_NS) exec -it deployment/service-tenant --container service-tenant -- bash

shell-db:
	@echo Use the following command line to drop into the db service:
	@echo   $(KC) -n $(APP_NS) exec -it deployment/teamadb --container teamadb -- bash
...
registry-login:
	...
	@/bin/sh -c 'echo ${CR_PAT} | $(DK) login $(CREG) -u $(DOCKER_REG) --password-stdin'
...
landlord: rollout-landlord cluster/service-landlord-svc.yaml cluster/service-landlord-sm.yaml cluster/service-landlord-vs-$(LANDLORD_VER).yaml
	$(KC) -n $(APP_NS) apply -f cluster/service-landlord-svc.yaml | tee $(LOG_DIR)/landlord.log
	$(KC) -n $(APP_NS) apply -f cluster/service-landlord-sm.yaml | tee -a $(LOG_DIR)/landlord.log
	$(KC) -n $(APP_NS) apply -f cluster/service-landlord-vs-$(LANDLORD_VER).yaml | tee -a $(LOG_DIR)/landlord.log

# Update S2 and associated monitoring, rebuilding if necessary
property: $(LOG_DIR)/property.repo.log cluster/service-property.yaml cluster/service-property-sm.yaml cluster/service-property-vs.yaml
	$(KC) -n $(APP_NS) apply -f cluster/service-property.yaml | tee $(LOG_DIR)/property.log
	$(KC) -n $(APP_NS) apply -f cluster/service-property-sm.yaml | tee -a $(LOG_DIR)/property.log
	$(KC) -n $(APP_NS) apply -f cluster/service-property-vs.yaml | tee -a $(LOG_DIR)/property.log

# Update S3 and associated monitoring, rebuilding if necessary
tenant: $(LOG_DIR)/tenant.repo.log cluster/service-tenant.yaml cluster/service-tenant-sm.yaml cluster/service-tenant-vs.yaml
	$(KC) -n $(APP_NS) apply -f cluster/service-tenant.yaml | tee $(LOG_DIR)/tenant.log
	$(KC) -n $(APP_NS) apply -f cluster/service-tenant-sm.yaml | tee -a $(LOG_DIR)/tenant.log
	$(KC) -n $(APP_NS) apply -f cluster/service-tenant-vs.yaml | tee -a $(LOG_DIR)/tenant.log
...
cri: $(LOG_DIR)/landlord-$(LANDLORD_VER).repo.log $(LOG_DIR)/property.repo.log $(LOG_DIR)/tenant.repo.log $(LOG_DIR)/db.repo.log

# Build the landlord service
$(LOG_DIR)/landlord-$(LANDLORD_VER).repo.log: ../code/service-landlord/$(LANDLORD_VER)/Dockerfile ../code/service-landlord/$(LANDLORD_VER)/app.py ../code/service-landlord/$(LANDLORD_VER)/requirements.txt
	$(DK) build -t $(CREG)/$(REGID)/team-a-service-landlord:$(LANDLORD_VER) ../code/service-landlord/$(LANDLORD_VER) | tee $(LOG_DIR)/landlord-$(LANDLORD_VER).img.log
	$(DK) push $(CREG)/$(REGID)/team-a-service-landlord:$(LANDLORD_VER) | tee $(LOG_DIR)/landlord-$(LANDLORD_VER).repo.log

# Build the property service
$(LOG_DIR)/property.repo.log: ../code/service-property/Dockerfile ../code/service-property/app.py ../code/service-property/requirements.txt
	$(DK) build -t $(CREG)/$(REGID)/team-a-service-property:$(APP_VER_TAG) ../code/service-property | tee $(LOG_DIR)/property.img.log
	$(DK) push $(CREG)/$(REGID)/team-a-service-property:$(APP_VER_TAG) | tee $(LOG_DIR)/property.repo.log

# Build the tenant service
$(LOG_DIR)/tenant.repo.log: ../code/service-tenant/Dockerfile ../code/service-tenant/app.py ../code/service-tenant/requirements.txt
	$(DK) build -t $(CREG)/$(REGID)/team-a-service-tenant:$(APP_VER_TAG) ../code/service-tenant | tee $(LOG_DIR)/tenant.img.log
	$(DK) push $(CREG)/$(REGID)/team-a-service-tenant:$(APP_VER_TAG) | tee $(LOG_DIR)/tenant.repo.log

# Build the db service
$(LOG_DIR)/db.repo.log: ../code/db/Dockerfile ../code/db/app.py ../code/db/requirements.txt
	$(DK) build -t $(CREG)/$(REGID)/team-a-teamadb:$(APP_VER_TAG) ../code/db | tee $(LOG_DIR)/db.img.log
	$(DK) push $(CREG)/$(REGID)/team-a-teamadb:$(APP_VER_TAG) | tee $(LOG_DIR)/db.repo.log

# Build the loader
$(LOG_DIR)/loader.repo.log: loader/app.py loader/requirements.txt loader/Dockerfile
	$(DK) image build -t $(CREG)/$(REGID)/team-a-cmpt756loader:$(LOADER_VER) loader  | tee $(LOG_DIR)/loader.img.log
	$(DK) push $(CREG)/$(REGID)/team-a-cmpt756loader:$(LOADER_VER) | tee $(LOG_DIR)/loader.repo.log
